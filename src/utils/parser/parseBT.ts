...
import { isBtBoilerplate, isBtTxStart } from './boilerplate';
import { buildBtDescription } from './descriptions';
import { autoCategory } from './autoCategory';

// ─── BT Personal account parser ──────────────────────────────────────────────

export function parseBT(pages: PdfPage[]): NewTransaction[] {
  ...
  for (const pageItems of pages) {
    ...
    for (const block of blocks) {
      ...
      const description = buildBtDescription(fullDesc, type);
