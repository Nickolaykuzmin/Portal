...
export default function Categories({ onMenuClick }: CategoriesProps) {
  ...
  return (
    <>
            ...
            {filtered.map((cat) => (
              <div key={cat.id} className="whisper-shadow" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: cat.color + '20', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                    <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 10, fontSize: 11, fontWeight: 600, background: cat.type === 'income' ? '#dcfce7' : '#fee2e2', color: cat.type === 'income' ? 'var(--secondary)' : 'var(--tertiary)' }}>
                      {cat.type === 'income' ? 'Дохід' : 'Витрата'}
                    </span>
                    {cat.isDefault && <span style={{ fontSize: 10, color: 'var(--outline)', fontWeight: 500 }}>стандартна</span>}
                  </div>
                </div>
              </div>
            ))}
      ...
      {showForm && (
        <div
          style={{ position: 'fixed', inset: 0, background: 'rgba(19,27,46,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 16 }}
          onClick={(e) => e.target === e.currentTarget && setShowForm(false)}
        >
          <div className="whisper-shadow" style={{ background: 'white', borderRadius: 20, padding: 32, width: '100%', maxWidth: 480, border: '1px solid var(--outline-variant)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: 'var(--surface-container-low)', borderRadius: 12 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: form.color + '20', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--on-surface-variant)', marginBottom: 6 }}>Назва</label>
                <input style={inputStyle} value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="Назва категорії" required />
              </div>

              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--on-surface-variant)', marginBottom: 6 }}>Тип</label>
                <div style={{ display: 'flex', gap: 8 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--on-surface-variant)', marginBottom: 8 }}>Іконка</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {ICONS.map((icon) => (
                    <button key={icon} type="button" onClick={() => setForm((f) => ({ ...f, icon }))} style={{
                      ...
                      <span className="material-symbols-outlined" style={{ fontSize: 16, color: form.icon === icon ? form.color : 'var(--outline)' }}>{icon}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--on-surface-variant)', marginBottom: 8 }}>Колір</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {COLORS.map((color) => (
                    <button key={color} type="button" onClick={() => setForm((f) => ({ ...f, color }))} style={{
                      width: 28, height: 28, borderRadius: '50%', background: color,
                      border: form.color === color ? '3px solid var(--on-surface)' : '2px solid transparent', cursor: 'pointer',
                    }} />
                  ))}
                  <input type="color" value={form.color} onChange={(e) => setForm((f) => ({ ...f, color: e.target.value }))}
                    style={{ width: 28, height: 28, borderRadius: '50%', border: 'none', cursor: 'pointer', padding: 0 }} title="Власний колір" />
                </div>
              </div>

              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                <button type="button" onClick={() => setShowForm(false)} style={{ flex: 1, padding: '12px', borderRadius: 10, border: '1px solid var(--outline-variant)', background: 'white', color: 'var(--on-surface)', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
                  Скасувати
                </button>
                <button type="submit" style={{ flex: 1, padding: '12px', borderRadius: 10, border: 'none', background: 'var(--primary)', color: 'white', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
                  Зберегти
                </button>
              </div>
            </form>
